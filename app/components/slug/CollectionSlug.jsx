import { getCommunitySlug, getDeveloperSlug, getTwitchinSlug } from "~/lib/collectionsQuery"
import { Link, useLoaderData } from "@remix-run/react"
import imageUrlBuilder from "@sanity/image-url"
import { client } from "~/lib/sanity"

export const loader = async () => {
    let communitySlug = await getCommunitySlug()
    let twitchinSlug = await getTwitchinSlug()
    let developerSlug = await getDeveloperSlug()
...
export default function CollectionSlug() {
    ...
    const collectionSlugArray = [communitySlug, twitchinSlug, developerSlug]
        ...
        <div className="bg-white py-24 sm:py-32">
            <div className="mx-auto max-w-7xl px-6 lg:px-8">
                ...
                <div className="mt-11 grid grid-cols-1 items-start gap-x-6 gap-y-16 sm:mt-16 sm:grid-cols-2 lg:grid-cols-4 lg:gap-x-8">
                    {collectionSlugArray.map((item) => (
                        <div key={item.title} className="flex flex-col-reverse">
                            <div className="mt-6">
                                <Link to="." className="relative flex items-center justify-center rounded-md border border-transparent bg-white px-8 py-2 text-sm font-medium text-gray-900 hover:bg-logopink"
